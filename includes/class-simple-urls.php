...
class Simple_Urls {
	...
	/**
	 * Register Post Type.
     * add a new attribute to table post_type
	 */
	...
	public function count_and_redirect() {
        ...
        // Validate the token if url contain param t else continue with predetermine behavior of simple urls
        $token = isset($wp_query->post->ID) ? get_post_meta($wp_query->post->ID, '_surl_token', true) : null;

        if(!isset($_GET['t']) && $token != '' || isset($_GET['t']) && $token != $_GET['t']){
             $redirect = '';
         }
